    ...
    >
      ...
      {isMenuOpen && (
        <motion.div
          ...
          exit={{ opacity: 0, y: -10 }}
          className="md:hidden mt-4 bg-white/95 backdrop-blur-md rounded-2xl shadow-lg p-4 space-y-2 border border-gray-100"
        >
          {["home", "about", "services", "projects", "contact"].map(
            (section) => (
              <button
                key={section}
                onClick={() => scrollToSection(section)}
                className={`block w-full text-left capitalize px-4 py-3 rounded-lg font-medium transition-all duration-200 ${
                  activeSection === section
